...
//CoderSchema.index({type: 1, location: 1});
CoderSchema.index({ location: '2dsphere' });
CoderSchema.index({ loc: '2dsphere' });
CoderSchema.index({ type: 'Point' });
...
CoderSchema.path('title').required(true, 'Coder title cannot be blank');
CoderSchema.path('body').required(true, 'Coder body cannot be blank');

/**
 * Pre-remove hook
 */

CoderSchema.pre('remove', function (next) {
  /*
