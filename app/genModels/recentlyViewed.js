...
//RecentlyViewedSchema.index({type: 1, location: 1});
RecentlyViewedSchema.index({ location: '2dsphere' });
RecentlyViewedSchema.index({ loc: '2dsphere' });
RecentlyViewedSchema.index({ type: 'Point' });
...
RecentlyViewedSchema.path('title').required(true, 'RecentlyViewed title cannot be blank');
RecentlyViewedSchema.path('body').required(true, 'RecentlyViewed body cannot be blank');

/**
 * Pre-remove hook
 */

RecentlyViewedSchema.pre('remove', function (next) {
  /*
...
RecentlyViewedSchema.methods = {

  /**
   * Save recentlyViewed and upload image
   *
   * @param {Object} images
   * @param {Function} cb
   * @api private
...
RecentlyViewedSchema.statics = {

  /**
   * Find recentlyViewed by id
   *
   * @param {ObjectId} id
   * @param {Function} cb
   * @api private
