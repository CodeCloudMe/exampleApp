...
//DetailSchema.index({type: 1, location: 1});
DetailSchema.index({ location: '2dsphere' });
DetailSchema.index({ loc: '2dsphere' });
DetailSchema.index({ type: 'Point' });
...
DetailSchema.path('title').required(true, 'Detail title cannot be blank');
DetailSchema.path('body').required(true, 'Detail body cannot be blank');

/**
 * Pre-remove hook
 */

DetailSchema.pre('remove', function (next) {
  /*
