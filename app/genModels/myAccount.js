...
//MyAccountSchema.index({type: 1, location: 1});
MyAccountSchema.index({ location: '2dsphere' });
MyAccountSchema.index({ loc: '2dsphere' });
MyAccountSchema.index({ type: 'Point' });
...
MyAccountSchema.path('title').required(true, 'MyAccount title cannot be blank');
MyAccountSchema.path('body').required(true, 'MyAccount body cannot be blank');

/**
 * Pre-remove hook
 */

MyAccountSchema.pre('remove', function (next) {
  /*
...
MyAccountSchema.methods = {

  /**
   * Save myAccount and upload image
   *
   * @param {Object} images
   * @param {Function} cb
   * @api private
...
MyAccountSchema.statics = {

  /**
   * Find myAccount by id
   *
   * @param {ObjectId} id
   * @param {Function} cb
   * @api private
...
mongoose.model('MyAccount', MyAccountSchema);
