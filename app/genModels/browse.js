...
//BrowseSchema.index({type: 1, location: 1});
BrowseSchema.index({ location: '2dsphere' });
BrowseSchema.index({ loc: '2dsphere' });
BrowseSchema.index({ type: 'Point' });
...
BrowseSchema.path('title').required(true, 'Browse title cannot be blank');
BrowseSchema.path('body').required(true, 'Browse body cannot be blank');

/**
 * Pre-remove hook
 */

BrowseSchema.pre('remove', function (next) {
  /*
...
BrowseSchema.methods = {

  /**
   * Save browse and upload image
   *
   * @param {Object} images
   * @param {Function} cb
   * @api private
