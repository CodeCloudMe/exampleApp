...
//LearnMoreSchema.index({type: 1, location: 1});
LearnMoreSchema.index({ location: '2dsphere' });
LearnMoreSchema.index({ loc: '2dsphere' });
LearnMoreSchema.index({ type: 'Point' });
...
LearnMoreSchema.path('title').required(true, 'LearnMore title cannot be blank');
LearnMoreSchema.path('body').required(true, 'LearnMore body cannot be blank');

/**
 * Pre-remove hook
 */

LearnMoreSchema.pre('remove', function (next) {
  /*
...
LearnMoreSchema.methods = {

  /**
   * Save learnMore and upload image
   *
   * @param {Object} images
   * @param {Function} cb
   * @api private
...
LearnMoreSchema.statics = {

  /**
   * Find learnMore by id
   *
   * @param {ObjectId} id
   * @param {Function} cb
   * @api private
...
mongoose.model('LearnMore', LearnMoreSchema);
